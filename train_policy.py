...
def train_discrete(model, iterator, w, opt, args):
    model.train()
    ...
    for i_batch, batch in enumerate(iterator):
        ...
        if args.weighted_loss: 
            # Wighted Classification Loss
            t = t.to(DEVICE)
            criterion = nn.CrossEntropyLoss(weight = w)
            loss = criterion(out, t)  #used get_class_distribution to get w
        else: 
            t = t.to(DEVICE)
...
def main(args):
    ...
    inv_freq = np.where(args.class_dist == 0 , args.class_dist, 1/args.class_dist) #get the inverse frequency
    w =  torch.from_numpy(inv_freq)
    best_val_accuracy = 0
    ...
    for epoch in range(args.n_epochs):
        ...
        train_discrete(driving_policy, training_iterator, w, opt, args)

        # Evaluate the driving policy on the validation set
        new_val_accuracy = test_discrete(driving_policy, validation_iterator, opt, args)
